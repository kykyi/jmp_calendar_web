...
RSpec.describe 'Spreadsheets contain no errors' do
  ...
  describe 'UON Year 1' do
    ...
    let(:spreadsheet) do
      load_spreadsheet(uni, year,
                       'MEDI1101B Timetable Weeks Pre16 to 29 for Callaghan & Central Coast - 2024 v9.xlsx')
    end
  ...
  describe 'UON Year 2' do
    let(:uni) { 'UON' }
    let(:year) { 2 }
    let(:spreadsheet) { load_spreadsheet(uni, year, '2025 MEDI2101A Timetable - CANVAS-2.xlsx') }
