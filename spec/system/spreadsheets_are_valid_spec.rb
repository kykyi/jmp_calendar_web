...
RSpec.describe 'Spreadsheets contain no errors' do
  ...
  describe 'UON Year 1' do
    ...
    let(:spreadsheet) do
      load_spreadsheet(uni, year,
                       'MEDI1101B Timetable Week Pre-16 to 22 2024 - Callaghan & Central Coast v6.xlsx')
    end
  ...
  describe 'UON Year 2' do
    let(:uni) { 'UON' }
    let(:year) { 2 }
    let(:spreadsheet) { load_spreadsheet(uni, year, '2024 MEDI2101B Timetable - 30 Aug.xlsx') }
